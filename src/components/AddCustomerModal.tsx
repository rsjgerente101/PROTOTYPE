...
function SimplifiedPicker({
    ...
    return (
        <div className="rounded-md p-3">
                        ...
                        <Marker
                            ...
                            eventHandlers={{
                                dragend: (e) => {
                                    ...
                                    const p = m.getLatLng();
                                    onChange({ lat: p.lat, lon: p.lng, address: value.address, locationSet: true });
                                },
                            }}
                        />
                    )}
            ...
            <div className="text-xs text-gray-600 mb-1">Search a place or click on the map to pin the customer location</div>
...
export default function AddCustomerModal({ isOpen, onClose, onConfirm, depot }: AddCustomerModalProps) {
    const [count, setCount] = useState(1);
    const [pickers, setPickers] = useState<PickLocation[]>([{ locationSet: false }] as PickLocation[]);

    useEffect(() => {
        setPickers(Array.from({ length: Math.max(1, count) }, (_, i) => pickers[i] ?? { locationSet: false }));
    ...
    const allValid = pickers.length > 0 && pickers.every((p) => p.lat != null && p.lon != null && p.locationSet);
                ...
                <div className="relative w-full max-w-4xl bg-white rounded-lg ring-1 ring-gray-200 z-[1501] flex flex-col max-h-[90vh]">
                    ...
                    <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                        {Array.from({ length: count }).map((_, idx) => (
                            <div key={idx} className="border border-slate-200 rounded p-3 overflow-hidden relative z-0">
